...
class gg {
  ...
  init() async {
    ...
    inv = await Hive.openBox('inventoryv2.2.0');
  ...
  List<Map<String, dynamic>> getProducts() {
    ...
    // print(inv.values.elementAt(7).toJson());
    for (Product element in inv.values) {
  ...
  List<Map<String, dynamic>> getOwners() {
    List<Map<String, dynamic>> result = List.empty(growable: true);
    for (Owner element in owners.values) {
      result.add(element.toJson());
    }
  ...
  List<Map<String, dynamic>> getLoaner() {
    List<Map<String, dynamic>> result = List.empty(growable: true);
    for (Loaner element in loaners.values) {
      result.add(element.toMap());
    }
    return result;
  }
